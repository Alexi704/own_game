...
if __name__ == '__main__':
    ...
    while check_answer() is False:
        ...
        # user_input = 'животные 300'  # временная строчка (после удалим)
        ...
        if user_selection(user_input) is False:
            ...
            if is_question(user_category, user_price) is False:
                ...
                user_answer = input(is_question(user_category, user_price))  # проверяем наличие темы и цены, а затем
                # получаем ответ от пользователя
                print(checking_user_answer(user_answer, user_category, user_price))  # проверяем ответ пользователя
                # и начисляем баллы
